...
private Document xmlDoc;
	...
	{
		Event event = new Event(4,true,100,100,Event.Sport.Football,"M","Rouge et or","Vert et or","Qu饕ec","Laval",new Date(),new Date());
	...
	public boolean writeTicketsToEvent(int _eventId, List<Ticket> _ticketsToAdd){
		...
		try {
			...
			//on a trouv�un event valide alors on ajoute les billets
			...
			//馗rire le contenu au fichier xml physique
			saveDataToFile();
	...
	private boolean saveDataToFile(){
		...
		try {
			...
			//propri騁馥s pour l'indentation du fichier
			transformer.setOutputProperty(OutputKeys.INDENT, "yes");
			transformer.setOutputProperty("{http://xml.apache.org/xslt}indent-amount", "2");
			
			//transformer le stream en 馗riture sur le fichier
	...
	public boolean modifyTicket(Ticket _ticket){
		...
		try {
			...
			//Trouver le ticket �l'int駻ieur de l'event
