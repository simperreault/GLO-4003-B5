...
public class SearchEngine {
	...
	public List<Ticket> findAllTickets(UUID _eventId) {
		return filterSoldTickets(dataManager.findEvent(_eventId)
				.getTicketList());
	}

	public List<Event> findAllEvents() {
		return filterSoldTicketsForAllEvents(dataManager
				.findAllEventsWithAllTickets());
	}
	 ...
	 */
	public List<Event> SearchWithCriterias(SportType _sport, int _days,
			String _team) {
	...
	private List<Event> filterTeam(String _team, List<Event> _list) {
		List<Event> returnList = new ArrayList<Event>();
		for (Event e : _list) {
			if (e.getHomeTeam().equals(_team)
					|| e.getVisitorsTeam().equals(_team)) {
				returnList.add(e);
			}
		}
		return returnList;
	 ...
	 */
	public List<Ticket> findSimilarTickets(List<Ticket> _ticketList,
			TicketType _type, String _section) {
	 ...
	 */
	public int countSimilarTickets(List<Ticket> _ticketList, TicketType _type,
			String _section) {
	...
	public List<ArrayList<Ticket>> regroupSimilarTickets(UUID _eventId) {
		...
		returnList.add((ArrayList<Ticket>) findSimilarTickets(ticketList,
				TicketType.GENERAL, ""));
	...
	public List<ArrayList<Ticket>> regroupSimilarTicketsByEvents(
		...
		for (ArrayList<Ticket> ar : eventSortedList) {
			...
			returnList.add((ArrayList<Ticket>) findSimilarTickets(ar,
					TicketType.GENERAL, ""));
			// then add simple tickets
			for (String s : e.listExistingSections()) {
				returnList.add((ArrayList<Ticket>) findSimilarTickets(ar,
	 ...
	 * @return
	 */
	public List<Ticket> filterListWithList(List<Ticket> _toFilter,
			List<Ticket> _filter) {
	...
	public List<Ticket> filterSoldTickets(List<Ticket> _toFilter) {
		List<Ticket> returnList = new ArrayList<Ticket>();
		for (Ticket t : _toFilter) {
			if (t.getOwner().isEmpty()
					|| (!t.getOwner().isEmpty() && t.getResellprice() != 0)) {
				returnList.add(t);
			}
		}
		return returnList;
