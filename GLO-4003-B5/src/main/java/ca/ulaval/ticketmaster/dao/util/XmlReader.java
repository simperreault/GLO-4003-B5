...
public class XmlReader {
	...
	public int[] readStartupInformation() {
		...
		Element elElement = ((Element) doc.getElementsByTagName("EventList")
				.item(0));
		output[0] = Integer.parseInt(elElement.getAttribute("total"));
		output[1] = Integer.parseInt(((Element) doc.getElementsByTagName(
				"UserList").item(0)).getAttribute("total"));
		return output;
	}
			...
			if (UUID.fromString(trElement.getAttribute("id")).equals(_transactionId) &&
				...
				for (int eventIter = 0; eventIter < eNodeList.getLength(); eventIter++) {
					...
					for (int tickIter = 0 ; tickIter < tNodeList.getLength(); tickIter++){
						...
						returnList.add(this.loadTicket(UUID.fromString(eElement.getAttribute("id")), UUID.fromString(tElement.getAttribute("id")))) ;
					}
				}
	...
	private Ticket readTicket(Element _element, Event _event) {
		Ticket tempTicket = TicketFactory.CreateExistingTicket(
				UUID.fromString(_element.getAttribute("id")), _event,
	...
	private Event readEvent(Element _element, boolean _andTickets) {
		...
		tempEvent.setTicketsTotal(Integer.parseInt(_element
				.getAttribute("ticketsTotal")));
		tempEvent.setTicketsAvailable(Integer.parseInt(_element
				.getAttribute("ticketsAvailable")));
		tempEvent.setSport(SportType.valueOf(((Element) _element
				.getElementsByTagName("Sport").item(0)).getAttribute("name")));
		tempEvent.setGender(((Element) _element.getElementsByTagName("Sport")
				.item(0)).getAttribute("gender"));
		tempEvent.setHomeTeam(((Element) _element.getElementsByTagName("Teams")
				.item(0)).getAttribute("home"));
		tempEvent.setVisitorsTeam(((Element) _element.getElementsByTagName(
				"Teams").item(0)).getAttribute("visitors"));
		tempEvent.setLocation(((Element) _element.getElementsByTagName(
				"Location").item(0)).getAttribute("city"));
		tempEvent.setStadium(((Element) _element.getElementsByTagName(
				"Location").item(0)).getAttribute("stadium"));
		try {
			String date = ((Element) _element.getElementsByTagName("Location")
					.item(0)).getAttribute("date")
					+ " "
					+ ((Element) _element.getElementsByTagName("Location")
							.item(0)).getAttribute("time");
			tempEvent.setDate(new SimpleDateFormat("dd/MM/yyyy HH:mm",
					Locale.ENGLISH).parse(date));
			tempEvent.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm",
					Locale.ENGLISH).parse(date));
		...
		NodeList sNodeList = ((Element) _element.getElementsByTagName(
				"SectionList").item(0)).getElementsByTagName("Section");
		...
		// Load Ticket List
		if (_andTickets) {
			NodeList tNodeList = ((Element) _element.getElementsByTagName(
					"TicketList").item(0)).getElementsByTagName("Ticket");
			for (int tickIter = 0; tickIter < tNodeList.getLength(); tickIter++) {
				Element tElement = (Element) tNodeList.item(tickIter);
	...
	private User readUser(Element _element) {
		...
		tempuser.setPassword(((Element) _element.getElementsByTagName(
				"PersonalData").item(0)).getAttribute("password"));
		tempuser.setFirstName(((Element) _element.getElementsByTagName(
				"PersonalData").item(0)).getAttribute("firstName"));
		tempuser.setLastName(((Element) _element.getElementsByTagName(
				"PersonalData").item(0)).getAttribute("lastName"));
		tempuser.setEmail(((Element) _element.getElementsByTagName(
				"PersonalData").item(0)).getAttribute("email"));
		tempuser.setAccessLevel(User.AccessLevel.valueOf(((Element) _element
				.getElementsByTagName("PersonalData").item(0))
				.getAttribute("accessLevel")));
		tempuser.setFavSport(((Element) _element.getElementsByTagName(
				"SearchPreferences").item(0)).getAttribute("sport"));
		tempuser.setFavGender(((Element) _element.getElementsByTagName(
				"SearchPreferences").item(0)).getAttribute("gender"));
		tempuser.setFavType(TicketType.valueOf(((Element) _element
				.getElementsByTagName("SearchPreferences").item(0))
				.getAttribute("type")));
		tempuser.setFavLocation(((Element) _element.getElementsByTagName(
				"SearchPreferences").item(0)).getAttribute("city"));

		NodeList tNodeList = ((Element) _element.getElementsByTagName(
				"UserTickets").item(0)).getElementsByTagName("Ticket");
		for (int tickIter = 0; tickIter < tNodeList.getLength(); tickIter++) {
			Element tElement = (Element) tNodeList.item(tickIter);
	...
	public Ticket loadTicket(UUID _eventId, UUID _ticketId) {
		connect(DATA_FILE);
		NodeList EventNodeList = ((Element) doc.getElementsByTagName("EventList").item(0)).getElementsByTagName("Event");
		for (int eventIter = 0; eventIter < EventNodeList.getLength(); eventIter++) {
			Element eElement = (Element) EventNodeList.item(eventIter);
			if (UUID.fromString(eElement.getAttribute("id")).equals(_eventId)) {
				// if event exists, check if ticket exists
				NodeList tNodeList = ((Element) eElement.getElementsByTagName(
						"TicketList").item(0)).getElementsByTagName("Ticket");
				for (int tickIter = 0; tickIter < tNodeList.getLength(); tickIter++) {
					Element tElement = (Element) tNodeList.item(tickIter);
					if (UUID.fromString(tElement.getAttribute("id")).equals(
							_ticketId)) {
	...
	public User userAuthenticate(String _username) {
		...
		for (int userIter = 0; userIter < UserNodeList.getLength(); userIter++) {
			...
			if ((uElement.getAttribute("username")).toLowerCase().equals(
